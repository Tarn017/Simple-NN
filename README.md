...
2. Kopiert den Ordner "tiere400", "requirements_class.txt`, das Skript "NeuralNetwork.py" und "main_FFN.py" euer Python-Projekt.  
3. Öffnet nun das Skript `main_FFN.py`. Zunächst müssen, falls noch nichtgeschehen, die nötigen Bibliotheken herunter geladen werden. Gehr dafür unten auf das Terminal und gebt ein `pip install -r requirements_class.txt`:
